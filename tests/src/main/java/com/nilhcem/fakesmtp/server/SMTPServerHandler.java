...
public enum SMTPServerHandler {
	...
	private final SMTPServer.Builder smtpServerBuilder = new SMTPServer.Builder().simpleMessageListener(myListener).authenticationHandlerFactory(new SMTPAuthHandlerFactory());
	...
	public void startServer(int port, InetAddress bindAddress) throws BindPortException, OutOfRangePortException {
		...
		try {
			smtpServer = smtpServerBuilder.bindAddress(bindAddress).port(port).build();
			smtpServer.start();
	...
	public boolean isRunning() {
		return smtpServer != null ? smtpServer.isRunning() : false;
	}
}
